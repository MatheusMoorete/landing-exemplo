"use client";

import { motion } from "framer-motion";
import { useInView } from "framer-motion";
import { useRef } from "react";
import { EditableText } from "@/components/admin/EditableText";

export function Footer() {
    const ref = useRef(null);
    const isInView = useInView(ref, { once: true, margin: "-50px" });

    return (
        <footer
            ref={ref}
            className="bg-charcoal"
        >
            <div className="container-narrow py-16">
                <motion.div
                    initial={{ opacity: 0, y: 20 }}
                    animate={isInView ? { opacity: 1, y: 0 } : {}}
                    transition={{ duration: 0.6 }}
                    className="grid gap-12 md:grid-cols-3"
                >
                    {/* Brand */}
                    <div>
                        <p className="mb-4 font-display text-xl text-white">
                            <EditableText contentKey="footer.brandName" />
                        </p>
                        <p className="text-sm leading-relaxed text-white/60">
                            <EditableText contentKey="footer.brandDesc" multiline />
                        </p>
                    </div>

                    {/* Links */}
                    <div>
                        <h4 className="mb-4 font-display text-lg text-white">
                            <EditableText contentKey="footer.linksTitle" />
                        </h4>
                        <ul className="space-y-3 text-sm">
                            <li>
                                <a
                                    href="#conteudo"
                                    className="text-white/60 transition-colors hover:text-terracotta-400"
                                >
                                    Conteúdo
                                </a>
                            </li>
                            <li>
                                <a
                                    href="#inscricao"
                                    className="text-white/60 transition-colors hover:text-terracotta-400"
                                >
                                    Preços
                                </a>
                            </li>
                            <li>
                                <a
                                    href="#"
                                    className="text-white/60 transition-colors hover:text-terracotta-400"
                                >
                                    Política de Privacidade
                                </a>
                            </li>
                            <li>
                                <a
                                    href="#"
                                    className="text-white/60 transition-colors hover:text-terracotta-400"
                                >
                                    Termos de Uso
                                </a>
                            </li>
                        </ul>
                    </div>

                    {/* Contact */}
                    <div>
                        <h4 className="mb-4 font-display text-lg text-white">
                            <EditableText contentKey="footer.contactTitle" />
                        </h4>
                        <a
                            href="mailto:contato@boloscontemporaneos.com"
                            className="mb-6 flex items-center gap-2 text-sm text-white/60 transition-colors hover:text-terracotta-400"
                        >
                            <svg
                                className="h-4 w-4"
                                fill="none"
                                viewBox="0 0 24 24"
                                stroke="currentColor"
                            >
                                <path
                                    strokeLinecap="round"
                                    strokeLinejoin="round"
                                    strokeWidth={2}
                                    d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"
                                />
                            </svg>
                            <EditableText contentKey="footer.contactEmail" />
                        </a>

                        <p className="mb-3 text-sm text-white/40">Redes Sociais</p>
                        <div className="flex gap-3">
                            <a
                                href="#"
                                className="flex h-10 w-10 items-center justify-center rounded-full border border-white/20 text-white/60 transition-all hover:border-terracotta-400 hover:text-terracotta-400"
                            >
                                <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24">
                                    <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z" />
                                </svg>
                            </a>
                            <a
                                href="#"
                                className="flex h-10 w-10 items-center justify-center rounded-full border border-white/20 text-white/60 transition-all hover:border-terracotta-400 hover:text-terracotta-400"
                            >
                                <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24">
                                    <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                                </svg>
                            </a>
                            <a
                                href="#"
                                className="flex h-10 w-10 items-center justify-center rounded-full border border-white/20 text-white/60 transition-all hover:border-terracotta-400 hover:text-terracotta-400"
                            >
                                <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24">
                                    <path d="M23.498 6.186a3.016 3.016 0 00-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 00.502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 002.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 002.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z" />
                                </svg>
                            </a>
                        </div>
                    </div>
                </motion.div>

                {/* Divider */}
                <div className="my-8 h-px bg-white/10" />

                {/* Bottom */}
                <motion.div
                    initial={{ opacity: 0 }}
                    animate={isInView ? { opacity: 1 } : {}}
                    transition={{ duration: 0.6, delay: 0.2 }}
                    className="text-center text-sm text-white/40"
                >
                    <p>© {new Date().getFullYear()} <EditableText contentKey="footer.copyright" /></p>
                    <p className="mt-2 text-white/30">
                        <EditableText contentKey="footer.hotmartDisclaimer" multiline />
                    </p>
                </motion.div>
            </div>
        </footer>
    );
}
